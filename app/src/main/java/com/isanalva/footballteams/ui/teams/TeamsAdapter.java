...
public class TeamsAdapter extends RecyclerView.Adapter<TeamsAdapter.TeamViewHolder> {
    ...
    private List<FootballTeam> items;
    ...
    public TeamsAdapter(List<FootballTeam> items) {
        this.items = items;
    }
    ...
    public void onBindViewHolder(TeamViewHolder viewHolder, int i) {
        String urlImg = TeamLogo.getInstance().getUrlTeamLogo(items.get(i).getShortName());
        Picasso.with(FootballTeamsApp.getContext()).load(urlImg).into(viewHolder.teamPhoto);
        viewHolder.teamName.setText(items.get(i).getName());
        viewHolder.teamMarketValue.setText(String.valueOf(items.get(i).getSquadMarketValue()));
    }
}
